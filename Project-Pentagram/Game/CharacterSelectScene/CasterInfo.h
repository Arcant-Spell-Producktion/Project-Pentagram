...
{
	R"(Fortified: When a unit with fortified effect take damage 
	from spell, turn it to 0 and decrease 1 stack.)",

	R"(Pertify: Unit that suffer from pertify will have 1 less 
	will for each spell.(can't go below 1))"
};
...
{
	R"(Marking: When a unit with mark effect take damage 
	from spell, Double it.)",
	
	R"(Burning: Unit with burning will take 10 damage per stack 
...
const std::string WaterEffectInfo[2] =
{
	...
	R"(Freezing: Unit that suffer from Freezing will use 1 more 
	mana to adjust cast time.(this effect is stackable))"
};
