...
TimetrackUI::TimetrackUI(int index,SpellTimetrack* track):m_TrackPtr(track),m_ObjectManager(GameStateController::GetInstance()->currentScene), UIObject("TrackUI_"+index)
{
    this->position.x = index * 125.0f - 625.0f;
    this->color.a = 0.0f;

    m_Box = m_ObjectManager->CreateUIObject("TrackUI_Box_" + index);
    m_Box->scale = { 125.0f,140.0f,1.0f };
    m_Box->color = AC_RED;
    m_Box->color.a = 0.5f + (0.5f * index / 10.0f);
    this->SetChildRenderBack(m_Box);

    m_PreviewIcon = m_ObjectManager->CreateObject<SpellIconUI>(new SpellIconUI("PreviewIcon_"+index));
